...
all.coef <- function(miss.data, aux.ind) {
  ...
  for (i in 1:length(all.x)) {
    ...
    if (all.x[i] == "x3" | all.x[i] == "x5" | all.x[i] == "x7" | all.x[i] == "x9") {
      ...
      mean.model <- lm_imp(as.formula(paste(all.x[i], "~ x1 + x2")), data = miss.data, 
                           auxvars = NULL, n.chains = 2, n.adapt  = 1000, 
                           n.iter = 3000, thin = 1, seed = i, mess = FALSE)
      
      beta.matrix <- as.matrix(mean.model$MCMC)[, c(1,2,3)]
      x.matrix <- c(1, 5, 0.5)
      mu <- invlogit(as.numeric(beta.matrix %*% x.matrix))
      ...
      coef.model <- lm_imp(as.formula(paste(all.x[i], "~", paste(coef.list[[i]]))),
                           data = miss.data, auxvars = as.formula(paste("~", paste(aux.list[[i]]))), 
                           n.chains = 2, n.adapt  = 1000, n.iter = 3000, thin = 1,
                           seed = i, mess = FALSE)
      
      coef.summary <- summary(coef.model)$res[[all.x[i]]]$regcoef
      ...
      mean.model <- glm_imp(as.formula(paste(all.x[i], "~ x1 + x2")), data = miss.data,
                            family = binomial(), auxvars = NULL, 
                            n.chains = 2, n.adapt  = 1000, n.iter = 3000, thin = 1, 
                            seed = i, mess = FALSE)
      ...
      coef.model <- glm_imp(as.formula(paste(all.x[i], "~", paste(coef.list[[i]]))),
                            data = miss.data, family = binomial(),
                            auxvars = as.formula(paste("~", paste(aux.list[[i]]))), 
                            n.chains = 2, n.adapt  = 1000, n.iter = 3000, thin = 1,
                            seed = i, mess = FALSE)
...
set.seed(123)
fbjm.mcar <- full.bayes(1000, "MCAR", 50, "Full")

set.seed(123)
fbjm.mar <- full.bayes(1000, "MAR", 50, "Full")

set.seed(123)
fbjm.mcar1 <- full.bayes(1000, "MCAR", 50, "Con")

set.seed(123)
fbjm.mar2 <- full.bayes(1000, "MAR", 50, "Con")
